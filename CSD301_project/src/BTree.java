...
public class BTree {
    ...
    void breadthFirstOrder() {
        ...
        while (!queue.isEmpty()) { //Dieu kien sai
            Node p = queue.pop();
            for(int i=0;i<p.mChildNodes.length;i++){
                if(p.mChildNodes[i] != null){
                    queue.push(p.mChildNodes[i]);
                }
            }
            for(int i=0;i<p.mNumKeys;i++){
                System.out.print(p.mObjects[i].toString() + "\n");
            }
        }
    }
    
    String breadthFirstOrderString() {
        String s = "";
        MyQueue<Node> queue = new MyQueue<>();
        if (isEmpty()) {
            return s;
        ...
        while (!queue.isEmpty()) { //Dieu kien sai
            ...
            for(int i=0;i<p.mNumKeys;i++){
                s = s.concat(p.mObjects[i].toString() + "\n");
            }
        }
        return s;
    }
